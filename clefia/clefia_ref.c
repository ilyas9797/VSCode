...
u8 rkdata[8 * 26 + 16]; /* 8 bytes x 26 rounds(max) + whitening keys */


void __stdcall setup (const u8 *skey)
{
  const u8 iv[2] = {0xb5, 0xc0U}; /* cubic root of 5 */
  u8 lk[32];
  u8 con256[4 * 92];
  int i;
...
void __stdcall crypt (u8 *pt, int r)
{
  ...
  ByteCpy(pt, rout, 16);
  ByteXor(pt + 4,  pt + 4,  rk + r * 8 + 0, 4); /* final key whitening */
  ByteXor(pt + 12, pt + 12, rk + r * 8 + 4, 4);
}
...
void __stdcall decrypt (u8 *ct, int r)
{
  ...
  ByteCpy(ct, rout, 16);
  ByteXor(ct + 4,  ct + 4,  rk - 8, 4); /* final key whitening */
  ByteXor(ct + 12, ct + 12, rk - 4, 4);
}